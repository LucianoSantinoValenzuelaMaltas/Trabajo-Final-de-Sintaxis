...
int main(int argc, char * argv[]) {
    ...
    if ( nomArchi[l-1] != 'm' || nomArchi[l-2] != '.' ){     //Verifica que sea .m el archivo
        printf("Nombre incorrecto del Archivo Fuente\n");
        return -1;
    }
...
TOKEN scanner()
{
    ...
    switch ( estado ){
        case 2 : if ( col != 11 ) //porque no puede ser un espacio en blanco
	        {
                ungetc(car, in);
	            buffer[i-1] = '\0';
	        }
   	    return ID;
        case 4 : if ( col != 11 ) //porque no puede ser un espacio en blanco
	        {
	            ungetc(car, in);
	            buffer[i-1] = '\0';
	        }
