...
#include <stdio.h> //Biblioteca indispensable en c, ya que contiene desde archivos, hasta las impresiones por pantalla
#include <string.h> //Para obtener las funciones de strings, por ejemplo la strcopy, que se utiliza para obtener el nombre del archivo micro
#include <ctype.h> //Usada para incorporar funciones de evaluación de caracteres, requeridas puntualmente en la matriz para obtener dígitos y letras
...
FILE * in;
typedef enum {
    INICIO, FIN, LEER, ESCRIBIR, ID, CONSTANTE, PARENIZQUIERDO, PARENDERECHO, PUNTOYCOMA, COMA, ASIGNACION, SUMA, RESTA, FDT, ERRORLEXICO, MULTIPLICACION, COCIENTE, MODULO_O_RESTO
} TOKEN;
typedef struct {
    char identifi[TAMLEX];
    TOKEN t;	/* t=0, 1, 2, 3 Palabra Reservada, t=ID=4 Identificador */
} RegTS; //Tabla de símbolos
RegTS TS[1000] = { {"inicio", INICIO}, {"fin", FIN}, {"leer", LEER}, {"escribir", ESCRIBIR}, {"$", 99} }; //La coma separa cada caso de struct que realizamos en la llaves
...
int main(int argc, char * argv[]) {
    ...
    if ( argc == 1 ){                                        //Constata que se incluya un archivo como parámetro
        printf("Debe ingresar el nombre del archivo fuente (en lenguaje Micro) en la linea de comandos\n");
        return -1;
    }
    if ( argc != 2 ){                                        //Verifica que sea únicamente 1 archivo
        printf("Numero incorrecto de argumentos\n");        
        return -1;
    }
    strcpy(nomArchi, argv[1]);
    l = strlen(nomArchi);
    if ( l > TAMNOM ){                                       //Chequea que el tamaño de su nombre (incluído el .m), no supere los 21 caracteres
        printf("Nombre incorrecto del Archivo Fuente\n");    
        return -1;
    }
    if ( nomArchi[l-1] != 'm' || nomArchi[l-2] != '.' ){     //Verifica que sea .m
        printf("Nombre incorrecto del Archivo Fuente\n");
        return -1;
    }

    if ( (in = fopen(nomArchi, "r") ) == NULL ){             //Constata que el archivo pueda abrirse en modo lectura
...
void Sentencia(void)
{
    ...
    switch ( tok ){
        case ID :	/* <sentencia> -> ID := <expresion> #asignar ; */
            Identificador(&izq);    //Guarda el identificador izquierdo
            Match(ASIGNACION);      //Busca hallar el operador asignación
            Expresion(&der);        //Guarda el identificador derecho
            Asignar(izq, der);      //Le asigna el valor del identificador derecho al identificador izquierdo
            Match(PUNTOYCOMA);      //Busca hallar el caracter de puntuación punto y coma
            break;
        case LEER :	/* <sentencia> -> LEER ( <listaIdentificadores> ) */
            Match(LEER);            //Busca hallar la aplabra reservada leer
            Match(PARENIZQUIERDO);  //Busca hallar el caracter de puntuación paréntesis izquierdo
            ListaIdentificadores();
            Match(PARENDERECHO);    //Busca hallar el caracter de puntuación paréntesis derecho
            Match(PUNTOYCOMA);      //Busca hallar el caracter de puntuación punto y coma
            break;
        case ESCRIBIR :	/* <sentencia> -> ESCRIBIR ( <listaExpresiones> ) */
            Match(ESCRIBIR);        //Busca hallar la aplabra reservada escribir
            Match(PARENIZQUIERDO);  //Busca hallar el caracter de puntuación paréntesis izquierdo
            ListaExpresiones();
            Match(PARENDERECHO);    //Busca hallar el caracter de puntuación paréntesis derecho
            Match(PUNTOYCOMA);      //Busca hallar el caracter de puntuación punto y coma
...
void Expresion(REG_EXPRESION * presul)
{
    ...
    /* <expresion> -> <primaria> { <operadorMultiplicativo> <primaria> #gen_infijo } */
    ...
    for ( t = ProximoToken(); t == SUMA || t == RESTA || t == MULTIPLICACION || t == COCIENTE  || t == MODULO_O_RESTO; t = ProximoToken() ){
        OperadorAditivo(op);
...
void OperadorMultiplicativo(char * presul)
{
    /* <operadorMultiplicativo> -> MULTIPLICACION #procesar_op | COCIENTE #procesar_op | MODULO_O_RESTO #procesar_op */

    TOKEN t = ProximoToken();

    if ( t == MULTIPLICACION || t == COCIENTE  || t == MODULO_O_RESTO ){
        Match(t);
        strcpy(presul, ProcesarOp());
    }
    else 
...
TOKEN scanner()
{
    int tabla[NUMESTADOS][NUMCOLS] = { {  1,  3,  5,  6,  7,  8,  9, 10, 11, 14, 13,  0, 14 },
				    ...
				    { 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14 },
				    { 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14 } };
    int car; //char es un subconjunto de int, así que sigue siendo char, y se pone int porque el caracter fdt escapa de char
    int col;
    int estado = 0;
